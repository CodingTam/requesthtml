    ...
    $('#new-request-tab').on('click', function() {
        setTimeout(() => {
            if (userInfo.userName) {
                $('#requestorName').val(userInfo.userName);
            }
...
function saveUserInfo() {
    ...
    .then(data => {
        if (data.success) {
            ...
            if (userInfo.userName) {
                $('#current-user').text(userInfo.userName);
                localStorage.setItem('currentUser', userInfo.userName);
            }

            showSuccessMessage('User Information Saved!', 'Your user information has been saved successfully.');
        } else {
...
function renderRequestsTable() {
    ...
    const dataToRender = filteredRequests.length > 0 || $('#searchRequests').val() || $('#statusFilter').val() ? filteredRequests : requests;
    ...
    dataToRender.forEach(request => {
        ...
        const row = `
            <tr onclick="viewRequestDetails('${request.request_id}')" style="cursor: pointer;" title="Click to view details">
                <td><strong>${request.request_id}</strong></td>
                <td>${request.requestor_name}</td>
                <td>${formattedDate}</td>
                <td>
                    <span class="status-badge ${statusClass}">
                        <i class="${statusIcon} mr-1"></i>${request.status}
                    </span>
                </td>
    ...
    // Initialize tooltips for details links only (pipeline uses native title)
    setTimeout(() => {
        // Only initialize tooltips for the details links
        $('.details-link[data-toggle="tooltip"]').tooltip({
            html: true,
            container: 'body',
            placement: 'left',
            trigger: 'hover'
...
function generateRequestDetailsBlob(request) {
    ...
    // Add key request information in a compact format
    if (request.requestor_email) details.push(`📧 Email: ${request.requestor_email}`);
    if (request.cc_email) details.push(`📧 CC: ${request.cc_email}`);
    if (request.team_name) details.push(`👥 Team: ${request.team_name}`);
    if (request.acct_number) details.push(`🏦 Account: ${request.acct_number}`);
    if (request.request_name) details.push(`📝 Name: ${request.request_name}`);
    if (request.request_dates) {
        const dateCount = request.request_dates.split(',').length;
        const datesList = request.request_dates.split(',').map(d => d.trim()).join(', ');
        details.push(`📅 Dates (${dateCount}): ${datesList}`);
    }
    if (request.description) {
        details.push(`💬 Description: ${request.description}`);
    }
    ...
    const detailsHtml = `
        <div class="row">
            <div class="col-md-6">
                ...
                <p><strong>Email:</strong> ${request.requestor_email}</p>
                <p><strong>CC Email:</strong> ${request.cc_email || 'N/A'}</p>
                <p><strong>Team:</strong> ${request.team_name}</p>
            </div>
            <div class="col-md-6">
                <h6>Request Details</h6>
                <p><strong>Category:</strong> ${request.category_name}</p>
                <p><strong>Account Number:</strong> ${request.acct_number}</p>
                <p><strong>Name:</strong> ${request.request_name}</p>
                <p><strong>Amount:</strong> ${formatCurrency(request.amount, request.currency)}</p>
                <p><strong>Dates:</strong> ${request.request_dates}</p>
            </div>
        </div>
        ${request.description ? `<div class="mt-3"><h6>Description</h6><p>${request.description}</p></div>` : ''}
    `;
...
function loadAdminDashboard() {
    ...
    Promise.all([
        ...
        fetch(`${API_BASE_URL}/admin/trends`).then(r => r.json())
    ])
    .then(([analytics, statusHistory, recentActivity, trends]) => {
        updateAdminSummaryCards(analytics.data);
        updateAdminBreakdownTables(analytics.data, statusHistory.data);
        updateAdminRecentActivity(recentActivity.data);
        updateAdminTrendsChart(trends.data);
        hideLoadingSpinner();
    })
    .catch(error => {
...
function updateAdminSummaryCards(data) {
    $('#admin-total-users').text(data.totalUsers || 0);
    $('#admin-daily-active').text(data.dailyActiveUsers || 0);
    $('#admin-total-requests').text(data.totalRequests || 0);
    $('#admin-daily-requests').text(data.dailyRequests || 0);
    $('#admin-completed-rate').text((data.completedRate || 0) + '%');
    $('#admin-failed-rate').text((data.failedRate || 0) + '%');
    $('#admin-total-status-changes').text(data.totalStatusChanges || 0);
    $('#admin-recent-changes').text(data.recentChanges || 0);
}
...
function updateAdminBreakdownTables(analytics, statusHistory) {
    ...
    if (statusHistory && statusHistory.length > 0) {
        statusHistory.slice(0, 10).forEach(change => {
            ...
            const row = `
                <tr>
                    <td><code>${change.statusChange}</code></td>
...
function updateAdminRecentActivity(activities) {
    ...
    if (activities && activities.length > 0) {
        activities.forEach(activity => {
            const changeDate = new Date(activity.change_datetime).toLocaleString();
            const changeType = activity.old_status ?
                `${activity.old_status} → ${activity.new_status}` :
                `Created as ${activity.new_status}`;

            const timelineItem = `
                <div class="timeline-item mb-3 p-2 border-left border-primary">
                    <div class="d-flex justify-content-between align-items-start">
                        <div>
                            <strong>${activity.request_id}</strong>
                            <br>
                            <small class="text-muted">${changeType}</small>
                            <br>
                            <small>by ${activity.changed_by || activity.requestor_name}</small>
                        </div>
...
function updateAdminTrendsChart(trendsData) {
    ...
    // Sort data by month (oldest first for proper chronological order)
    const sortedData = trendsData.sort((a, b) => a.month.localeCompare(b.month));

    const labels = sortedData.map(d => d.month);
    const requestCounts = sortedData.map(d => d.requestCount || 0);
    const completedCounts = sortedData.map(d => d.completedCount || 0);
    const failedCounts = sortedData.map(d => d.failedCount || 0);
...
function filterRequestsByStatus(status) {
    ...
    if (status === 'all') {
        ...
        const matchingRows = rows.filter(function() {
            ...
            const $row = $(this);
            const statusCell = $row.find('td').eq(3); // Status is the 4th column (0-indexed)

            // First try to find by badge text
            let statusBadge = statusCell.find('.status-badge, .badge');
            if (statusBadge.length === 0) {
                // If no badge found, try any span in the status column
                statusBadge = statusCell.find('span');
            }

            let statusText = '';
            if (statusBadge.length > 0) {
                statusText = statusBadge.text().toLowerCase().trim();
            } else {
                // Fallback to cell text
                statusText = statusCell.text().toLowerCase().trim();
            }

            console.log('Row HTML:', $row.html().substring(0, 200));
            console.log('Status cell HTML:', statusCell.html());
            console.log('Original status text:', statusText);

            // Clean up the status text
            if (statusText.includes('submitted')) statusText = 'submitted';
            else if (statusText.includes('processing')) statusText = 'processing';
            else if (statusText.includes('completed')) statusText = 'completed';
            else if (statusText.includes('failed')) statusText = 'failed';

            console.log('Cleaned status text:', statusText, 'Looking for:', status);
            const matches = statusText === status;
...
function updateTableHeaderCount() {
    const visibleRows = $('#requestsTableBody tr:visible').length;
    const totalRows = $('#requestsTableBody tr').length;

    if (currentFilter === 'all') {
        $('#requests-card-title').text(`All Requests (${totalRows})`);
    } else {
        const statusText = currentFilter.charAt(0).toUpperCase() + currentFilter.slice(1);
        $('#requests-card-title').text(`${statusText} Requests (${visibleRows})`);
    }
}
